...
Excogitation is a book review website where users get to resgister (own an account) and view reviews of others; they also can review and rate a book they choose.

This project has a folder structure where it constitutes the custom cascading style sheets(css) files in a folder named css included in a folder named static. There are two custom css files; the front-end is the result of these custom css files and bootstrap 4. The folder templates include all the html files that get rendered to the user. The file named 404.html is a page that displays page not found when a user requests the api for displaying the data in a json format with a wrong ISBN number input on the url. signin.html and login.html deals with registering and logging in users respectively by appending and searching data in the database. Flask_wtf is used for the forms in each html file through out, which takes care of validation and creating of the fields in the file named wtform_fields.py. The file named file_helper.html stands as a midway and/or template file which helps in displaying the forms in each html file in an easy and more ordered manner by making use of macros. The file layout.html stands as a template file in which each html file except 404.html and form_helper.html extends from, its a file where the basic structure of an html file is defined in. The file home.html is the home page where the user is first directed to. The file named index.html is the first page the user resides with as the user registers/logs in where the user is given a chance to search for a book of preference; the user can search inputting (title of the book, name of the author or even entering the ISBN number). Following the resulting lists from the inputted search, by clicking a single book another page is redirected where details.html holds the content. Details.html is the page where the user can check previous posted reviews and ratings by other users and if the user hasn't posted any review on the book yet a form gets displayed encouraging the user to post a review for the book; if the user has already posted a review, the form won't be visible so that the user won't review or rate this specific book for the second time. Above the form in details.html, reviews on the specific book will be listed, if a review from the list was made by the user who has logged in, the user has the privilege of editing or even deleting the review that she/he has posted. In the details.html and edit.html files the user can be directed to the search page (index.html) by clicking on 'Excogitation' on the navbar simplifying navigation from one page to the other. In each of the html files except for the 404.html, form_helper.html, layout.html and home.html each file displays the user that is currently logged in and constitutes the functionality of logging out too. The file edit.html is the page that gets rendered when the user clicks on the edit link after posting a review. This page includes a form with a placeholder of the previous posted review so that the user can refer the previously posted review before updating the content of the review. 

The file import.py is where the tables (users, books and reviews) are created and where data from the file books.csv gets migrated to the books table by first establishing a connection with the database. The other file is the main flask file application.py; this file includes around 10 functions with a specific route with the functionalities of first rendering the home.html file, signing in/registering (inserting data to the users table after validation), logging in (checking credentials from the users table and input for a valid sign in), logging out (made use of session), searching (from the books table by matching with either of the title, author or ISBN), displaying details (displays the reviews made by altering the reviews table; inserting data when a user posts a review and when previously posted reviews get displayed to the user by retrieving from the reviews table), editing a post (a posted review can be edited by the user who did so by altering the reviews table), deleting a post ( a posted review can be removed by the user who had posted it), error loader (page not found, an error page that gets rendered when a user requests the api with a wrong ISBN number), api requesting (by passing a valid ISBN number a JSON format will be displayed with the appropriate data by fetching data from the books and reviews table). And lastly requirements.txt is the file where all the dependencies for this project are included in so that its installed before running this program. 
